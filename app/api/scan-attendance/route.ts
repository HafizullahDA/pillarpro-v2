  ...
  try {
    ...
    if (authError || !user) {
      return NextResponse.json(
        { error: 'Authentication required. Please sign in to scan attendance.' },
        { status: 401 }
      )
    }
    ...
    if (!canCreateAttendance(userRole as string | null)) {
      return NextResponse.json(
        { error: 'Forbidden. Your role does not have permission to mark or scan attendance.' },
        { status: 403 }
      )
    }
    ...
    if (!rateLimit.success) {
      return NextResponse.json(
        {
          error: `Rate limit reached. You can scan up to ${RATE_LIMIT_CONFIG.limit} attendance sheets every 2 minutes. Please wait ${rateLimit.resetSeconds}s.`,
        },
        {
          status: 429,
          headers: {
    ...
    if (imageBase64.length > 6 * 1024 * 1024) {
      return NextResponse.json(
        { error: 'Muster roll image is too large (> 4.5 MB). Please take a smaller photo or compress it.' },
        { status: 413 }
      )
    }
...
5. Return valid JSON only with no markdown or codeblocks.`
    ...
    return NextResponse.json(
      { error: err.message || 'Failed to analyze muster roll image' },
      { status: 500 }
    )
  }
}
